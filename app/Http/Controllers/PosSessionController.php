...
//             $cashSales    = $breakdown['cash'] ?? $breakdown['cash_on_hand'] ?? 0;
//             $startingFund = $validated['starting_fund'] ?? $session->starting_fund ?? 0;
//             $tip          = $validated['tip'] ?? 0;
//             $transferOut  = $validated['transfer_amount'] ?? 0;
...
//                 'receivable'           => round($expectedCash, 2),
//                 'shortage'             => $shortage,
//                 'overage'              => $overage,
//                 'tip'                  => $tip,
...
foreach ($breakdown as $slug => $group) {
    ...
    if (empty($cleanBreakdown)) {
        $breakdown[$slug] = $breakdown[$slug]['total'];
            ...
            // Now 100% safe math
            $expectedCash = $startingFund + $cashSales + $tip - $transferOut;
            $countedCash  = $this->calculateTotalCashCounted($validated);
