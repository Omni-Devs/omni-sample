...
<div class="main-content">
    <div>
        <div class="breadcrumb">
            <h1 class="mr-3">Edit Customer</h1>
            <ul>
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <input type="text" name="customer_no" id="customer_no" 
                                                class="form-control" 
                                                value="{{ old('customer_no', $customer->customer_no ?? 'CUS-000') }}">
                                        </div>
                                        ...
                                        <div class="col-md-6">
                                            <fieldset class="form-group">
                                                ...
                                                <input type="text" name="landline_no" id="landline_no" 
                                                        ...
                                                        value="{{ old('landline_no', $customer->landline_no) }}">
                                            </fieldset>
                                        </div>
                                            ...
                                            <fieldset class="form-group">
                                                <legend class="col-form-label pt-0">Customer Since *</legend>
                                                    <input type="date" name="customer_since" id="customer_since" 
                                                        ...
                                                        value="{{ old('customer_since', $customer->customer_since) }}">
                                            </fieldset>
                                        </div>
                                        ...
                                        <div class="col-md-6">
                                            <fieldset class="form-group">
                                                ...
                                                <input type="text" name="assigned_personnel" id="assigned_personnel" 
                                                        ...
                                                        value="{{ old('assigned_personnel', $customer->assigned_personnel) }}">
                                            </fieldset>
                                        </div>
                                        ...
                                        <div class="col-md-6">
                                            <fieldset class="form-group">
                                                ...
                                                <select name="customer_type" id="customer_type" class="custom-select">
                                                    ...
                                                    <option value="regular_customer" {{ old('customer_type', $customer->customer_type) == 'regular_customer' ? 'selected' : '' }}>Regular Customer</option>
                                                    <option value="vip_customer" {{ old('customer_type', $customer->customer_type) == 'vip_customer' ? 'selected' : '' }}>VIP Customer</option>
                                                    <option value="walk_in_customer" {{ old('customer_type', $customer->customer_type) == 'walk_in_customer' ? 'selected' : '' }}>Walk In Customer</option>
                                                    <option value="senior_customer" {{ old('customer_type', $customer->customer_type) == 'senior_customer' ? 'selected' : '' }}>Senior Customer</option>
                                                    <option value="athlete_type" {{ old('customer_type', $customer->customer_type) == 'athlete_type' ? 'selected' : '' }}>Athlete Type</option>
                                                </select>
                                            </fieldset>
                                        </div>
                                                ...
                                                <div class="d-flex">
                                                    <select name="discount_id" id="discount_id" class="custom-select mr-2">
                                                        ...
                                                        @foreach ($discounts as $discount)
                                                            <option value="{{ $discount->id }}" {{ old('discount_id', $customer->discount_id) == $discount->id ? 'selected' : '' }}>
    ...
    0 - 0 of 0
                            ...
                            <div class="b-overlay-wrap position-relative d-inline-block btn-loader mt-3">
                                <button type="submit" class="btn btn-primary"><i class="i-Yes me-2"></i>Update Customer</button>
                            </div>
                        </div>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    const $city = $('#city_municipality');

    $province.select2({ placeholder: 'Select Province', allowClear: true, width: '100%' });
    $city.select2({ placeholder: 'Select City/Municipality', allowClear: true, width: '100%' });

    // Fetch all provinces dynamically from PSA API
    const provincesResponse = await fetch('https://psgc.gitlab.io/api/provinces/');
    const provinces = await provincesResponse.json();

    provinces.forEach(p => {
        const opt = new Option(p.name, p.name, false, false);
        $(opt).attr('data-code', p.code);
        $province.append(opt);
    });

    $province.trigger('change.select2');

    // Set default selected values when editing
    const selectedProvince = "{{ old('province', $customer->province ?? '') }}";
    const selectedCity = "{{ old('city_municipality', $customer->city_municipality ?? '') }}";

    if (selectedProvince) {
        $province.val(selectedProvince).trigger('change.select2');

        // Load cities based on selected province
        const provinceCode = $province.find('option:selected').data('code');
        if (provinceCode) {
            const citiesResponse = await fetch(`https://psgc.gitlab.io/api/provinces/${provinceCode}/cities-municipalities/`);
            const cities = await citiesResponse.json();
            cities.forEach(c => {
                const cityOpt = new Option(c.name, c.name, false, false);
                $city.append(cityOpt);
            });
    ...
    $province.on('change', async function () {
        ...
        $city.empty();
        if (!code) {
            $city.append(new Option('Select City/Municipality', '', false, false)).trigger('change.select2');
            return;
        }

        const resp = await fetch(`https://psgc.gitlab.io/api/provinces/${code}/cities-municipalities/`);
        const cities = await resp.json();
        $city.empty();
        cities.forEach(c => $city.append(new Option(c.name, c.name, false, false)));
