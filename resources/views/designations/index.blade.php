        ...
        <div class="card-body">
            <nav class="card-header">
                <ul class="nav nav-tabs card-header-tabs">
                    <li class="nav-item">
                        <a href="{{ route('designations.index', ['status' => 'active']) }}" class="nav-link {{ $status === 'active' ? 'active' : '' }}">Active</a>
                    </li>
                    <li class="nav-item">
                        <a href="{{ route('designations.index', ['status' => 'archived']) }}" class="nav-link {{ $status === 'archived' ? 'active' : '' }}">Archived</a>
                    </li>
                </ul>
            </nav>
                        ...
                        <div class="vgt-global-search__actions vgt-pull-right">
                            <div class="mt-2 mb-3">
                                ...
                                <button type="button" class="btn btn-rounded btn-btn btn-primary btn-icon m-1" 
                                        data-bs-toggle="modal" data-bs-target="#New_Designation">
                                    <i class="i-Add"></i> Add
                                </button>

                                <!-- Add Designation Modal -->
                                <div class="modal fade" id="New_Designation" tabindex="-1" role="dialog" aria-labelledby="New_DesignationLabel" aria-hidden="true">
                                    <div class="modal-dialog modal-lg" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="New_DesignationLabel">Add Designation</h5>
                        ...
                        <table id="vgt-table" class="table-hover tableOne vgt-table ">
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col" aria-sort="descending" aria-controls="col-1" class="gvt-left-align text-left sortable" style="min-width: auto; width: auto;">
                                        ...
                                        <button><span class="sr-only">Sort table by Designation Name in descending order</span></button>
                                    </th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="vgt-left-align text-left">{{ $designation->created_at->timezone('Asia/Manila')->format('Y-m-d H:i') }}</td>
                                    <td class="vgt-left-align text-left">{{ $designation->name }}</td>
                                    <td class="vgt-left-align text-left">{{ $designation->creator?->username ?? 'N/A' }}</td>
                                    <td class="vgt-left-align text-right">
                                        <div class="dropdown b-dropdown btn-group">
                                            <!-- 3-dot button -->
                                            <button id="dropdownMenu{{ $designation->id }}"
                                                type="button"
                                            ...
                                            <!-- Dropdown items -->
                                            <ul class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenu{{ $designation->id }}">
                                                <!-- Edit -->
                                                ...
                                                <!-- DELETE -->
                                                @if($designation->status === 'archived')
                                                <li role="presentation">
                                                ...
                                                @if($designation->status === 'active')
                                                <li role="presentation">
                                                    <form action="{{ route('designations.archive', $designation) }}" method="POST"
                                                ...
                                                @if($designation->status === 'archived')
                                                <li role="presentation">
                                                    <form action="{{ route('designations.restore', $designation) }}" method="POST"
                                ...
                                <!-- Edit Modal for this Designation -->
                                            ...
                                            <form action="{{ route('designations.update', $designation->id) }}" method="POST">
                                                ...
                                                <div class="modal-body">
                                                    ...
                                                    <div class="form-group mb-3">
                                                        <label for="designation-created-{{ $designation->id }}">Date &amp; Time Created</label>
                                                        <input type="datetime-local"
                                                                name="created_at"
                                                                id="designation-created-{{ $designation->id }}"
                                                                class="form-control @error('created_at') is-invalid @enderror"
                                                                value="{{ old('created_at', $designation->created_at ? $designation->created_at->timezone('Asia/Manila')->format('Y-m-d\TH:i') : '') }}">
                                                        @error('created_at')
                                                            <div class="invalid-feedback">{{ $message }}</div>
                                                        @enderror
                                                        <small class="form-text text-muted">
                                                    ...
                                                    <!-- Designation Name -->
                                                    <div class="form-group mb-3">
                                                        <label for="designation-name-{{ $designation->id }}">Designation Name *</label>
                                                        <input type="text"
                                                                name="name"
                    ...
                    <div class="vgt-wrap__footer vgt-clearfix">
                        ...
                        <div class="footer__navigation vgt-pull-right">
                            <div data-v-347cbcfa="" class="footer__navigation__page-info">
                                <div data-v-347cbcfa="">
                                    1 - {{ $designations->count() }} of {{ $designations->count() }}
                                </div>
                            </div>
