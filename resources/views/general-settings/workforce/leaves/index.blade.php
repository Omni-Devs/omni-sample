        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="leavesModalLabel">
                ...
                <div class="modal-body">
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control" v-model="leaveType" placeholder="Enter Name of Leave Type">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Notice Period (days)</label>
                            <input type="number" class="form-control" v-model="noticePeriod" placeholder="Enter Days">
                        </div>
                            ...
                            <table id="vgt-table"  class="table-hover tableOne vgt-table">
                                ...
                                <tr>
                                    ...
                                    <th scope="col" aria-sort="descending" aria-controls="col-3" class="vgt-left-align text-left w-160px sortable" style="min-width: auto; width: auto;"><span>Notice Period</span> <button><span class="sr-only">
                                        Sort table by Notice Period in descending order
                                        </span></button>
                                    </th>
                                    <!----><!----><!----><!----><!----><!----><!----><!---->
                                ...
                                <tbody>
                                    <tr v-for="row in filteredRecords" :key="row.id">
                                        ...
                                        <td class="vgt-left-align text-left w-160px">@{{ row.notice_period }}</td>
                                        <td class="vgt-left-align text-right">
                                            <actions-dropdown :row="row" 
                                                @edit-leave="openEditModal"
                                                @archive-leave="archiveLeave"
                                                @restore-leave="restoreLeave"
...
<div class="dropdown btn-group" ref="dropdown">
    ...
    <ul :class="['dropdown-menu dropdown-menu-right', { show: isOpen }]">
        <template v-if="row.status === 'archived'">
            <li>
                <a class="dropdown-item" @click.prevent="editLeave">
                    <i class="nav-icon i-Edit font-weight-bold mr-2"></i>
                    Edit
                </a>
            </li>
            <li>
                <a class="dropdown-item text-danger" href="#" @click.prevent="$emit('delete-leave', row.id)">
                    <i class="nav-icon i-Remove-Basket font-weight-bold mr-2"></i>
                    Permanently Delete
                </a>
            </li>
            <li>
                <a class="dropdown-item" href="#" @click.prevent="$emit('restore-leave', row.id)">
                    <i class="nav-icon i-Restore-Window font-weight-bold mr-2"></i>
                    Restore as Active
                </a>
            </li>
            <li>
                <a href="javascript:void(0);" class="dropdown-item" @click="$emit('open-remarks', leaveId)">
        ...
        <template v-else>
            ...
            <li>
                <a class="dropdown-item" href="#" @click.prevent="archiveLeave">
                    <i class="nav-icon i-Letter-Close font-weight-bold mr-2"></i>
                    Move to Archive
                </a>
            </li>
            <li>
                <a class="dropdown-item" :href="`/workforce-leaves/${leaveId}/logs`">
...
Vue.component("actions-dropdown", {
    ...
    methods: {
        ...
        archiveLeave() {
            // Emit event to parent with the leave ID
            this.$emit('archive-leave', this.row.id);
            this.isOpen = false; // close dropdown
        },
    ...
    mounted() {
        ...
        methods: {
            openAddModal() {
                ...
                const modalEl = document.getElementById('leavesModal');
            ...
            openEditModal(row) {
                ...
                this.leaveType = row.name;
                this.noticePeriod = row.notice_period;
            ...
            async archiveLeave(leaveId) {
                try {
                    const res = await axios.patch(`/workforce-leaves/${leaveId}/archive`);
            ...
            async restoreLeave(leaveId) {
                try {
                    const res = await axios.patch(`/workforce-leaves/${leaveId}/restore`);
                    Swal.fire({
                        icon: 'success',
                        title: 'Restored!',
                        text: res.data.message,
            ...
            async deleteLeave(leaveId) {
                const confirm = await Swal.fire({
                    icon: 'warning',
                    title: 'Are you sure?',
                    text: "This will permanently delete the leave!",
                ...
                try {
                    await axios.delete(`/workforce-leaves/${leaveId}`);
                    Swal.fire({
                        icon: 'success',
                        title: 'Deleted!',
            ...
            async saveLeave() {
                if (!this.leaveType) {
                    return Swal.fire("Required", "Leave name is required.", "warning");
                }
                if (this.noticePeriod === "" || this.noticePeriod === null) {
                    return Swal.fire("Required", "Notice period is required.", "warning");
                }

                let payload = {
                    name: this.leaveType.trim(),
                    notice_period: Number(this.noticePeriod),
                };
                ...
                try {
                    ...
                    if (this.isEditing) {
                        // UPDATE
                        res = await axios.put(`/workforce-leaves/${this.editId}`, payload);
                    } else {
                        // CREATE
                        res = await axios.post('/workforce-leaves', payload);
                    }
                    ...
                    this.fetchRecords();
                    bootstrap.Modal.getInstance(document.getElementById('leavesModal')).hide();
