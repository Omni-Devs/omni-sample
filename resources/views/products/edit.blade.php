   ...
   <table class="table table-bordered" id="recipeTable">
      ...
      <tbody>
         @foreach($oldRecipes as $i => $recipe)
            @php
               // support both array shape from old() and minimal arrays from product->recipes
               $recipeId = $recipe['id'] ?? '';
            ...
            <tr data-index="{{ $i }}">
               ...
               <td>
                  <select name="recipes[{{ $i }}][component_id]" class="form-control component-select" required>
                     @foreach($components as $component)
                        <option value="{{ $component->id }}" data-cost="{{ $component->cost }}"
               ...
               <td>
                  <input type="number" name="recipes[{{ $i }}][quantity]" step="0.01"
                     class="form-control recipe-quantity" value="{{ $quantity }}" required>
               </td>

               <td>
                  <input type="text" name="recipes[{{ $i }}][unit]" class="form-control" value="{{ $unit }}" required>
               </td>

               <td>
                  <input type="text" name="recipes[{{ $i }}][cost]" class="form-control component-cost"
    ...
    // components must be passed from controller as $components
      ...
      function createComponentOptionsHTML(selectedId = null) {
         return components.map(c => {
               ...
               return `<option value="${c.id}" data-cost="${c.cost}" ${sel}>${c.name}</option>`;
         }).join('');
      }
      ...
      function addRecipeRow(prefill = {}) {
         ...
         tr.innerHTML = `
               ...
               <td><input type="text" name="recipes[${i}][unit]" class="form-control" value="${unit}" required></td>
      ...
      function initRow(tr) {
         ...
         // do initial calculation for prefilled rows
         updateCost();
      }
