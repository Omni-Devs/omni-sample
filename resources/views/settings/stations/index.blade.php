...
<div class="main-content" id="app">
    <div>
        <div class="breadcrumb">
            <h1 class="mr-3">Stations</h1>
            <ul>
                <li><a href="">Kitchen Display Settings</a></li>
            </ul>
        </div>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="row g-3">
                        <div class="col-md-12">
                            <label class="form-label fw-bold">Created Date and Time</label>
                            <input
                            type="text"
                            class="form-control"
                        ...
                        <div class="col-md-12">
                            ...
                            <input type="text" class="form-control" v-model="stationType" placeholder="Enter Name of Station Type">
                        </div>
                        <div class="col-md-12">
                            <label class="form-label fw-bold">Description</label>
                            <input type="text" class="form-control" v-model="description" placeholder="Enter Description of Station Type">
                        </div>
                       
                        <div class="col-12 mt-4">
                            <button type="button" class="btn btn-primary" @click="saveStation">
                    ...
                    <div class="vgt-inner-wrap">
                        ...
                        <div class="vgt-responsive">
                            <table id="vgt-table"  class="table-hover tableOne vgt-table">
                                ...
                                <thead>
                                <tr>
                                    <th class="w-190px">Date and Time Created</th>            
                                ...
                                <tbody>
                                    <tr v-for="row in filteredRecords" :key="row.id">
                                        ...
                                        <td class="vgt-left-align text-left">@{{ row.description }}</td>
                                        <td class="vgt-left-align text-left">
                                            <actions-dropdown :row="row" 
                                                @edit-station="openEditModal"
                                                @archive-station="archiveStation"
...
<div class="dropdown btn-group" ref="dropdown">
    ...
    <ul :class="['dropdown-menu dropdown-menu-right', { show: isOpen }]">
        <template v-if="row.status === 'archived'">
            <li>
                <a class="dropdown-item" @click.prevent="editStation">
                    <i class="nav-icon i-Edit font-weight-bold mr-2"></i>
                    Edit
                </a>
            </li>
            <li>
                <a class="dropdown-item text-danger" href="#" @click.prevent="$emit('delete-station', row.id)">
                    <i class="nav-icon i-Remove-Basket font-weight-bold mr-2"></i>
                    Permanently Delete
                </a>
            </li>
            <li>
                <a class="dropdown-item" href="#" @click.prevent="$emit('restore-station', row.id)">
        ...
        <template v-else>
            <li>
                ...
                @click.prevent="editStation">
                    <i class="nav-icon i-Edit font-weight-bold mr-2"></i>
                    Edit
                </a>
            </li>
            <li>
                <a class="dropdown-item" href="#" @click.prevent="archiveStation">
                    <i class="nav-icon i-Letter-Close font-weight-bold mr-2"></i>
                    Move to Archive
                </a>
            </li>
            <li>
                <a class="dropdown-item" :href="`/settings/stations/${row.id}/logs`">
...
Vue.component("actions-dropdown", {
    ...
    methods: {
        editStation() {
            ...
            this.$emit('edit-station', this.row);
            this.isOpen = false; // close dropdown
        },
        archiveStation() {
            // Emit event to parent with the station ID
            this.$emit('archive-station', this.row.id);
            this.isOpen = false; // close dropdown
        },
...
new Vue({
    ...
    mounted() {
        this.fetchRecords();
        this.createdDateTime = this.getCurrentDateTime();
        },
      ...
      return `${year}-${month}-${day} ${hours}:${minutes}:${seconds}`;
            ...
            openAddModal() {
                ...
                const modalEl = document.getElementById('stationsModal');
            ...
            openEditModal(row) {
                ...
                this.stationType = row.name;

                const modalEl = document.getElementById('stationsModal');
                const modal = new bootstrap.Modal(modalEl);
                modal.show();
            },
            async archiveStation(stationId) {
                try {
                    const res = await axios.patch(`/settings/stations/${stationId}/archive`);
            ...
            async restoreStation(stationId) {
                try {
                    const res = await axios.patch(`/settings/stations/${stationId}/restore`);
                    Swal.fire({
                        icon: 'success',
                        title: 'Restored!',
                        text: res.data.message,
            ...
            async deleteStation(stationId) {
                const confirm = await Swal.fire({
                    icon: 'warning',
                    title: 'Are you sure?',
                    text: "This will permanently delete the station!",
                ...
                try {
                    await axios.delete(`/settings/stations/${stationId}`);
                    Swal.fire({
                        icon: 'success',
                        title: 'Deleted!',
            ...
            fetchRecords(page = 1) {
                axios.get("{{ route('stations.fetch') }}", {
                    params: {
            ...
            async saveStation() {
                if (!this.stationType) {
                    return Swal.fire("Required", "Station name is required.", "warning");
                }

                let payload = {
                    name: this.stationType.trim(),
                ...
                try {
                    ...
                    if (this.isEditing) {
                        // UPDATE
                        res = await axios.put(`/settings/stations/${this.editId}`, payload);
                    } else {
                        // CREATE
                        res = await axios.post('/settings/stations', payload);
                    }
                    ...
                    this.fetchRecords();
                    bootstrap.Modal.getInstance(document.getElementById('stationsModal')).hide();
